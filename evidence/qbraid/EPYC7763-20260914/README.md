...
The unpinned 32/48/64/96 sweep measured its best result at 48 workers for both Float and BAM-LUT. A repeat-matched 48-worker affinity study then found that node-local execution remained close to the unpinned baseline while a one-SMT-thread-per-exposed-core mask spanning both NUMA nodes was roughly 50% slower for both backends.
