...
// 64-bit tiled GALAXY kernels. UFF data/constants are prepended by build.rs.
struct Global {
    info: vec4<u32>,       // resident count, seed, completed steps, tile sample count
    ...
    phase_time: vec4<f32>, // time/TAU in four exact-product chunks
    address: vec4<u32>,    // tile base lo/hi, logical population lo/hi
}
...
fn components(r: f32) -> vec3<f32> {
    ...
    for (var i = 1u; i < DEMO_N; i++) {
        if r <= radii[i] {
            return mix(values[i-1u], values[i], (r-radii[i-1u])/(radii[i]-radii[i-1u]));
        }
    }
...
fn nfw_shape(x: f32) -> f32 {
    if x < 0.1 {
        return x*x*(0.5+x*(-2.0/3.0+x*(0.75+x*(-0.8+x*(5.0/6.0-x*6.0/7.0)))));
    }
    ...
    switch p.info.x {
        ...
        case 4u: {
            if total > 0.0 {
                ...
                var denominator: f32;
                if root < 0.1 {
                    denominator = root*(1.0+root*(-0.5+root*(1.0/6.0+root*(-1.0/24.0+root/120.0))));
                }
        ...
        case 5u: {
            ...
            var shape: f32;
            if x < 0.15 {
                shape = q*(1.0/3.0+q*(-0.2+q*(1.0/7.0+q*(-1.0/9.0+q/11.0))));
            }
...
fn global_id(local_index: u32) -> vec2<u32> {
    let lo = settings.address.x + local_index;
    let carry = select(0u, 1u, lo < settings.address.x);
    return vec2<u32>(lo, settings.address.y + carry);
}

fn address_word(local_index: u32, lane: u32) -> u32 {
    let id = global_id(local_index);
    let lane_key = (lane + 1u) * 0x9e3779b9u;
    let low_mix = hash32(id.x ^ settings.info.y ^ lane_key ^ hash32(id.y ^ 0xa511e9b3u));
    return hash32(low_mix ^ id.y*0x9e3779b9u ^ 0x85ebca6bu);
}

fn random_global(local_index: u32, lane: u32) -> f32 {
    return f32(address_word(local_index, lane) >> 8u)/16777216.0;
}

@compute @workgroup_size(256)
fn initialize(@builtin(global_invocation_id) id: vec3<u32>) {
    let i = id.x;
    if i >= settings.info.x { return; }
    let u = random_global(i,0u);
    let v = random_global(i,1u);
    let w = random_global(i,2u);
    let kind = random_global(i,4u);
    let bulge = kind < settings.shape.w;
    let halo = kind >= 0.97;
    var r = 0.06+0.92*pow(u,1.4);
    if bulge { r = 0.015+0.27*pow(u,1.8); }
    else if halo { r = 0.25+0.85*u; }
    var theta = TAU*v;
    if !bulge && !halo {
        theta = floor(v*settings.shape.x)*TAU/settings.shape.x
            + log(r/0.1)/tan(settings.shape.y)
            + (w-0.5)*settings.shape.z;
    }
    var height = settings.extra.x*(0.4+r);
    if bulge { height = 2.28*(1.0-r/0.3); }
    else if halo { height = 4.8; }
    let z = (random_global(i,3u)-0.5)*2.0*height;
    r *= 12.0;
    ...
    let omega = settings.motion.z*speed/r;
    let kick = settings.extra.y*KMS_TO_KPC_MYR;
    particles[i] = Particle(
        vec4<f32>(r,theta,z,omega),
        vec4<f32>(
            r*cos(theta),
            r*sin(theta),
            -omega*r*sin(theta)+kick*cos(theta),
            omega*r*cos(theta)+kick*sin(theta)
        )
    );
}
...
fn circular(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    particles[i].state = vec4<f32>(
        orbit.x*cos(theta),
        orbit.x*sin(theta),
        -orbit.w*orbit.x*sin(theta),
        orbit.w*orbit.x*cos(theta)
    );
}
...
fn gather(@builtin(global_invocation_id) id: vec3<u32>) {
    let i = id.x;
    if i >= settings.info.w { return; }
    let local_index = cases[i].info.y;
    outputs[i] = particles[local_index];
}
