...
//! Dependency-free deterministic reference math for GALAXY validation.
//! Inspired by Elite-style compact seed recurrence and Doom-style fixed/BAM
//! arithmetic. This crate is intentionally separate from production sampler,
//! Wasm and GPU crates.
...
fn rol32(value: u32, shift: u32) -> u32 { value.rotate_left(shift) }

/// Elite's three-word recurrence supplies the low-32 procedural grammar.
/// High sector bits are mixed separately by `sector_salt`, preventing the
/// historical recurrence period from becoming GALAXY's global identity period.
...
pub fn sector_salt(global_seed: u32, sector: u64) -> u32 {
    ...
    hash32(folded ^ hash32(lo ^ 0x9e37_79b9) ^ hash32(hi ^ 0x85eb_ca6b))
}

pub fn bam_add(angle: u32, delta: u32) -> u32 { angle.wrapping_add(delta) }
pub fn bam_sub(angle: u32, delta: u32) -> u32 { angle.wrapping_sub(delta) }
...
/// Return cosine/sine in signed Q2.30 using 24 integer CORDIC stages.
pub fn sin_cos_q30(angle: u32) -> (i64, i64) {
    let mut z = if angle < BAM_HALF { angle as i64 } else { angle as i64 - (1_i64 << 32) };
